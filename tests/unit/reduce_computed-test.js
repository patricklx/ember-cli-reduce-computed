...
test("changes to array computed properties happen asynchronously", function() {
  var nestedNumbers = get(obj, 'nestedNumbers');
  var evenNestedNumbers = get(obj, 'evenNestedNumbers');
  ...
  run(function() {
    nestedNumbers.objectAt(0).set('v', 22);
    deepEqual(nestedNumbers.mapBy('v'), [22, 2, 3, 4, 5, 6], 'nested numbers is updated');
    deepEqual(evenNestedNumbers, [2, 4, 6], 'do not add new number synchronously');
  });
  run(function () {
    deepEqual(evenNestedNumbers, [2, 4, 6, 22], 'adds new number asynchronously');
  });
});

test("changes to array computed properties are flushed on get", function() {
  var nestedNumbers = get(obj, 'nestedNumbers');
  var evenNestedNumbers = get(obj, 'evenNestedNumbers');
  ...
  run(function() {
    ...
    deepEqual(nestedNumbers.mapBy('v'), [22, 2, 3, 4, 5, 6], 'nested numbers is updated');
    deepEqual(evenNestedNumbers, [2, 4, 6], 'do not add new number synchronously');
    get(obj, 'evenNestedNumbers');
    deepEqual(evenNestedNumbers, [2, 4, 6, 22], 'adds new number after get');
  });
});
...
test("`updateIndexes` is not over-eager about skipping retain:n (#4620)", function() {
  ...
  obj = EmberObject.extend({
    ...
    items: arrayComputed('content.@each.n', {
      ...
      initialize: function (initialValue, changeMeta, meta) {
        meta.previousValues = {};
      },

      addedItem: function (array, item, changeMeta, meta) {
        meta.previousValues[guidFor(item)] = get(item, 'n');
        tracked.push('+' + get(item, 'n') + '@' + changeMeta.index);
        array.insertAt(changeMeta.index, item);
        return array;
      },
      removedItem: function (array, item, changeMeta, meta) {
        var prev = meta.previousValues[guidFor(item)];
        tracked.push('-' + (prev ? prev : get(item, 'n')) + '@' + changeMeta.index);
